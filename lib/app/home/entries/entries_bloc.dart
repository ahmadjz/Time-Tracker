...
class EntriesBloc {
  ...
  /// combine List<Job>, List<Entry> into List<EntryJobModel>
  Stream<List<EntryJobModel>> get _allEntriesStream => Rx.combineLatest2(
        database.entriesStream(),
        database.jobsStream(),
        _entriesJobsCombiner,
      );

  static List<EntryJobModel> _entriesJobsCombiner(
      List<Entry> entries, List<Job> jobs) {
    return entries.map((entry) {
      final job = jobs.firstWhere((job) => job.id == entry.jobId);
      return EntryJobModel(entry, job);
    }).toList();
  }
